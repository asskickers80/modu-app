...
## 스키마 의존 배포 (2026-08-04 대표 지시)
- 스키마 의존 코드는 컬럼 부재 시에도 저장이 실패하지 않도록 폴백 처리하거나,
  SQL 실행 확인 후 배포되도록 커밋을 분리한다.
- 배포 후 SQL 대기 상태에서 기존 기능이 깨지는 구성 금지.

## 양도인(E1) 보호장치 (ORDER-e1-district-draft-v2부터 상시)
- E1(양도인) 코드를 건드리는 오더는 착수 전 스냅샷 스위트(tests/e1-seller-snapshot.spec.js +
  기존 E1 계열 스펙)가 그린인 커밋을 먼저 남긴다.
- E1 공유 코드 수정 시 양도인 경로 동작 변경은 오더가 명시한 범위만. 그 외 리팩토링 포함 불변.
  공용 함수 시그니처 변경 시 기존 호출부 동작 보존을 diff로 증명.
- 보고에 "양도인 영향 diff 요약"(바꾼 것/안 바꾼 것 + 스냅샷 스위트 전후 그린) 섹션 의무.

## 서브에이전트
- 조사는 explorer, 테스트 실행은 test-runner를 사용한다.

## 향후 개발 아이디어 백로그 (상기 규칙)
- 새 축(창업준비·양도인·소유주·사장님·기업회원·방문자)의 화면을 새로 만들거나, 홈 피드를 바꾸거나,
  외부 서비스·업체 연결 기능을 만드는 작업을 시작할 때는 코드를 건드리기 전에
  docs/향후개발아이디어_백로그.md를 읽는다.
- 발동 조건에 맞는 항목이 있으면 첫 응답에서 "백로그 B-n: (한 줄 요약) — 이번 작업에 포함할까요?"라고
  대표에게 먼저 묻고, 답을 받은 뒤 진행한다. 조건에 맞는 항목이 없으면 아무 말도 하지 않는다.
- 항목을 구현하거나 대표가 폐기하면 그 파일의 상태 칸을 갱신하고 커밋에 포함한다.
...
- 모두에 질문하기의 답변 재료는 docs/principles/ASK_LOG.md와 2026-09-13 오더 A4의 화이트리스트 밖으로 넓히지 않는다. 권리금·매출·상권 평가는 어떤 경우에도 답하지 않는다.
- 질문 로그는 원문 90일·집계 영구다. 임베딩·벡터 저장과 user_id 직접 저장을 하지 않는다.
- 양도인이 문의 답장으로 쓴 문장을 매물 필드·소개글·다른 사용자 답변에 옮기지 않는다. 매물 정보는 양도인이 직접 등록·수정한 것만이다.
- 화면 이름은 '모두에 질문하기'다. 문안에 'AI'를 쓰지 않는다.
- 정체성·카피 세트·어휘 규칙의 단일 소스는 docs/BRAND.md 다. 화면 문안을 만들기 전에 그 파일을 읽고, 거기 없는 규칙을 다른 문서에 새로 적지 않는다.
- "리테일·생태계·플랫폼·인프라"는 대외 언어다. 사용자 대면 화면에 쓰지 않는다(허용 자리는 BRAND.md와 scripts/lint-brand.mjs 예외 목록에만 둔다).
- 무료 등급 모델에는 사용자 자유 입력·연락처·양도인 답장을 보내지 않는다. 프로덕션에서 자유 입력 생성 경로를 켜려면 유료 등급으로 전환한다(config/ai.ts의 tier).
- 매물 카드·매물 상세·알림 문안·'모두에 질문하기'를 만지기 전에 백로그 방향 원칙 '판매자 우선'을 읽는다. 타 사용자 화면에 등록일·가격 변동 이력·변동 횟수를 넣는 구현 요청이 오면 구현 전에 대표에게 이 원칙을 상기한다.
- 홈 피드 → 프로필 매칭 피드 전환을 시작할 때는 백로그 B-10(축별 매물 카드 강조 순서)을 첫 응답에서 대표에게 상기한다.
- 탐색 결과 화면·필터·빈 화면을 만지기 전에 config/searchRelax.ts의 완화 사다리와 '건수 0 후보는 만들지 않는다' 규칙을 확인한다. 정도 부사·판정 문구를 넣어 달라는 요청이 오면 구현 전에 대표에게 원칙을 상기한다.

 ...
 * 검사 대상: 이름에 sort/rank/order/card/route/ask 가 들어가는 함수 본문 (src/, config/).
 */
...
const SORT_FN = /(?:function\s+(\w*(?:sort|rank|order|card|dispatch|assign|route|ask)\w*)\s*\([^)]*\)\s*\{)|(?:(?:const|let|var)\s+(\w*(?:sort|rank|order|card|dispatch|assign|route|ask)\w*)\s*=\s*(?:async\s*)?(?:\([^)]*\)|\w+)\s*=>\s*\{)/gi
...
export function findPricingSortViolations(source, file = '<inline>') {
  ...
  for (const m of source.matchAll(SORT_FN)) {
    ...
    const isSort = /sort|rank|order|dispatch|assign|route/i.test(name)

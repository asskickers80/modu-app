...
### 다음 할 일
- 업종 분류 통일 1단계 — **대표님 SQL 백필 실행 대기 (여기서 멈춤)**. 완료: categories.ts에 안경점·약국 소분류 신설, 매핑 확정판(docs/BIZTYPE-UNIFY-STEP0.md), 백필 SQL(scripts/sql/20260720-biztype-unify.sql — franchise_brands 11,683건 44종 전량 커버, 기계 대조 누락 0). 백필 후 재개: ③E1 업종 입력 A3 컴포넌트 재사용 → ④깨진 3곳(탐색 필터·시설 추천·뉴스 매칭) 정정 → ⑤홈 헤더 표기 → ⑥재질문 플로우(대상 6건) → ⑦완성도 채점

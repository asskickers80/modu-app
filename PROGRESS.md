...
- 시장 뉴스 자동 수집: Vercel Cron 매일 새벽 5시(KST) → /api/collect-market-news (배포 실호출 검증 — saved 65). 12일 묵은 기사 문제 해소.

### 다음 할 일
- D4 통합 인박스 설계·구현 (/messages 단일 화면 + [문의]/[업체 제안] 필터 딥링크 — 라우트 생기면 더보기 항목 자동 노출, 별도 세션)
- 홈 본문 CTA("매물 등록·수정하기")와 더보기 '내 매물 수정하기' 문구 역할 분담 (미결 — 채팅 후속 결정)

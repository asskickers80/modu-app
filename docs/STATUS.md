...
> 최종 업데이트: 2026-09-22 (기업회원 E1b 실저장 + 대기 SQL 전수 해소)  
> 빌드: ✅ `npm run build` 성공 · lint 4종(pricing·copy·brand·seller-first) 통과  
> 테스트: Playwright **829개** — 전체 PASS(1 skipped), 외부 API 실호출 0  
> 마지막 커밋: `fe9295e`  
> 현재 스냅샷·다음 할 일은 루트 `PROGRESS.md` 가 단일 현황판. 이 문서는 세션별 이력이다.

---

## 완료 (2026-09-14~22 — 브랜드·판매자 우선·탐색 완화·기업회원)

| 날짜 | 오더 | 내용 | SQL |
|---|---|---|---|
| 09-14 | 새 로고 적용 | 확정 원본 1장에서 자산 10종 파생·화면 배치(가로형/심볼/실루에트 자리 구분) | — |
| 09-15 | 판매자 우선 원칙 | 타 사용자 화면에서 등록일·가격 변동 이력 제거, 가격 알림 판매자 선택제, 최근 확인일 [아직 있어요] | ✅ 실행됨 |
| 09-16 | 심볼 벡터화 | 확정 로고 기하를 벡터로 고정·앱 전체 통일(작은 크기 뭉개짐 해결) | — |
| 09-21 | 탐색 조건 풀기·저장 조건 | 0건·소수건 완화 사다리(모델 호출 0)·저장 조건 새 매물 알림·수요 집계 | ✅ 실행됨 |
| 09-22 | 기업회원(E1b) 실저장 | 5단계 완료가 저장하도록 배선(`listing_type='business'`) + 업종·개업연도 입력 신설 + 더미 기본값 사망 | ⚠ 3칸 대기 |

### 이번 기간 교훈
- **"저장을 붙이기 전에 무엇이 저장되는지 본다"** — E1b는 insert 한 줄이면 될 줄 알았지만, Context 기본값이 더미였고 그걸 고칠 입력이 화면에 없었다. 그대로 붙였으면 모든 업체가 '서교동 인테리어·2019년'으로 저장돈다.
- **"소비처가 이미 구조를 정해 놓았다"** — 기업회원을 별도 표로 만들 뾻했으나, 배정·상세·한마디 3곳이 이미 `listings` + `listing_type='business'` 를 읽고 있었다. 쓰는 쪽을 만들기 전에 읽는 쪽을 먼저 읽는다.
- **"카피도 사실이어야 한다"** — '수정 불가 — 검증된 사실 기반'은 사용자 입력을 받는 순간 거짓말이 된다. 국세청에서 온 것과 직접 받는 것을 문안에서 나눔다.
- **"순수 룰은 supabase 를 끌어오면 안 된다"** — 저장 함수와 같은 파일에 둔 payload 조립 룰은 `import.meta.env` 때문에 유닛 테스트가 아예 안 돌았다. `*Rules.js` 분리가 기존 패턴인 이유.
- **"타이핑 최소화와 가짜 데이터는 다르다"** — 쓰기 어려운 칸은 기본값을 박을 게 아니라 placeholder 로 예시를 보여준다. 예시는 저장되지 않는다.
- **"SQL 은 채팅에 전문으로"** — 파일 경로로 넘기면 그 자리에서 작업이 멈췘다(대표 세 번째 지적).

### 다음 후보
- **기업회원 SQL 3칸 실행** (`biz_founded`·`biz_solutions`·`biz_settings`) + `listings.listing_type` CHECK 제약에 `'business'` 허용 여부 확인
- 실제 입점 1건으로 시세 문의 배정·`/e2b/:id` 상세·[전화하기] 실동작 확인 (저장이 생겼으니 이제 가능)
- Vercel `ASK_PSEUDONYM_SALT` 등록 (없으면 빈 솔트로 동작)
- 한국부동산원 `REB_API_KEY` 발급 → `api/_rebConnector.js` PLACEHOLDER 교체
- 실기기 테스트 완주(`docs/실기기테스트_따라하기.md`)
- PLACEHOLDER 확정: 후기 블라인드 30일 / quiet 30일·계정당 2개 / 한마디 72시간·5명

---

## 완료 (2026-09-09~13 — 9월 세션군)

| 날짜 | 오더 | 내용 | SQL |
|---|---|---|---|
| 09-09 | 서비스 카드·문의 채널·온보딩 규칙 | 매출 상황 카드 1장(룰만)·기업회원 [문의하기]/[전화하기] 이원화·문의 원장 | ✅ 실행됨 |
| 09-10 | 찜·다음 행동·완성도·정부 연결 | 찜 3종과 알림 6종·소유자 카드·내 관심 화면·다음 행동 카드·완성도 '다음 1개'·정부 지원 링크 | ✅ 실행됨 |
| 09-11 | 부동산원·상호로 시작하는 등록·시세 물어보기 | 부동산원 비교선(키 대기·모의 배치)·상호 한 칸 등록 자동 채움·'모두에 시세 물어보기' 파이프라인 | ✅ 실행됨 |
| 09-12 | 후기 2종·조용히 올리기·한마디 | 별점 없는 후기(양도인 삭제권·업체 이의신청)·quiet 서버 마스킹 뷰·기업회원 한마디 승인 3개 | ✅ 실행됨 |
| 09-13 | 모두에 질문하기·갱신 리포트·질문 로그 | 3갈래 라우팅 질문 창구·② 전달 4박자·원문 90일/집계 영구·기업회원 갱신 D-7 | ✅ 실행됨 |

### 이번 기간 교훈
- **"답변이 아니라 분류가 목적"** — 질문 창구의 값은 답을 만드는 데 있지 않다. 우리가 못 답하는 축이 무엇인지 드러나고, 그게 문의와 등록 항목으로 이어지는 구조가 자산이다.
- **"말한 것은 반드시 실행한다"** — "전달하고 안내할게요"라고 띄웠으면 전달 카드와 대화 열기가 실제로 붙어야 한다. 안내 문구만 있고 행동이 없는 구현은 정직 원칙 위반이다.
- **"무료 등급 모델에는 사용자 문장을 보내지 않는다"** — 약관상 무료 등급 입력은 사람이 읽을 수 있다. 기능은 룰로 돌리고, 유료 전환 한 줄로 생성 경로를 연다(config/ai.ts tier).
- **"원문은 90일, 구조는 영구"** — 지우기 전에 축·전환으로 굳혀 두면 파기 후에도 분석이 남는다. 임베딩·벡터·user_id 직접 저장은 하지 않는다.
- **"서버 마스킹은 클라이언트 숨김이 아니다"** — quiet 매물은 DB 뷰가 값을 빼고 응답한다(9/12). 화면에서 가리는 방식이었다면 네트워크 탭에서 그대로 보였다.
- **"lint 정규식의 \\s 는 줄을 먹는다"** — 문안 lint의 줄 번호가 어긋나 오탐을 냈다. 주석 제거는 줄 수를 보존해야 한다(9/13 수정).

### 다음 후보
- Vercel `ASK_PSEUDONYM_SALT` 등록 (질문 로그 pseudonym 회전 해시 — 없으면 빈 솔트로 동작)
- 기업회원 축(E1b) 실저장 — 찜·전화·시세 문의 배정·업체 상세·한마디가 전부 이 저장 하나에 걸려 있다
- 한국부동산원 `REB_API_KEY` 발급 → `api/_rebConnector.js` PLACEHOLDER 교체
- 실기기 테스트 58단계 완주(`docs/실기기테스트_따라하기.md`)
- PLACEHOLDER 확정: 후기 블라인드 30일·삭제 흔적 표시 / quiet 30일·계정당 2개 / 한마디 72시간·5명·링크 로그인 여부 / 질문 하루 10회·만료 7일 / 갱신 D-7·D-14·3개월·성사 3건

---

## 완료 (2026-07-09~17 — 102커밋, 주제별 요약)

| 주제 | 대표 커밋 | 내용 |
|------|------|------|
| **하늘·구름 디자인 시스템** | (7/9~) | A1 스플래시(2초, 하늘 배경+화이트 심볼)·A2 구름 카드 톤 전면 적용, 역할별 페이지 배경 미세 틴트 4%, 태양 색 "모두" 강조, 아이폰/아이패드 풀블리드 |
| **A3 양도자 온보딩 개편** | b670801, b05b196, f3c88a0, 8245ef9 | 단일 카드 3문항 + 섹션 접힘/펼침. 업종 2단계 드릴다운(`categories.ts` — 8대분류+소분류+KSIC 대조+동의어 검색), 지역 2단계 드릴다운(`regions.ts` — 전국 17시도+226기초단체), 목적 문항(transfer_priority). profiles/listings 분류 3컬럼 SQL 적용(대표님 콘솔) |
| **카카오 로그인 실동작** | c2b7c43, 159cc36 | kauth/kapi CORS 미허용 → Vercel 서버 함수(`api/kakao-auth.js`) 경유 토큰 교환. redirect_uri 정식 주소 고정(`lib/appOrigin.js`)으로 배포별 KOE006 해소. 카카오 신콘솔 리다이렉트 등록(대표님). **실기기 로그인 성공 확인** |
| **네이버 로그인 실동작** | dec5653, 786fcdd | 정상 설정 앱 신규 생성(PC 웹 환경) + 서버 함수(`api/naver-auth.js`) + state CSRF 검증. 키 미설정 환경(로컬·테스트)은 더미 통과 유지 |
| **"진짜 로그인" — 계정 데이터 연속성** | a506a2d | 계정 기준 기기 ID(auth user_metadata.device_id) 동기화 — 어느 브라우저에서 로그인해도 매물·메시지 동일. 로그인 전 데이터 병합, 로그아웃 시 기기 ID 초기화 |
| **이메일 로그인 마무리 + A4 탭 구조** | a7334ad, c373886~6d245fc | 인증 에러 한국어화, 재설정 메일 정식 주소 고정. A4 로그인/회원가입 밑줄 탭(온보딩 경유=회원가입, 지름길=로그인), 소셜 버튼 슬림화 |
| **A2 다중 선택 B안** | 96fb99e, 1b4e319, 72af6b1 | 대표 역할만 온보딩, 나머지는 pending 멀티프로필 자동 등록 → 전환 시 A3 보완 모드(`?complete=1&pid=`). 목록은 localStorage(카카오 앱 전환 왕복 생존). 방문자는 중복 선택 배제 |
| **프로필 칩 + 전면 스와이프 전환** | 5d0a16c, 0a44197, 964ef83 | 공용 ProfileChips(활성=라벨 칩, 비활성=색 점), 화면 전체 좌우 스와이프로 즉시 전환(`useProfileSwipe`), 목록 끝에선 전환·추가 시트 |
| **칩·화면 불일치 구조적 차단** | a1e8878, 335745f | pending 전환을 질문 완료 시점에 확정 + `useProfileRouteSync`(뒤로가기·복원 시 라우트 기준 자동 복구) — iOS 엣지 스와이프 뒤로가기 불일치 재발 방지 + 회귀 테스트 |
| **역할 용어 통일** | 81bd43f | 온보딩 기준으로 통일: 양도인·소유주·창업자·사장님·기업회원·방문자 (칩·대시보드·폴백 문구 전체) |
| **가입/로그인 의도 명확화** | 05e0bac | 로그아웃 시 로컬 완전 초기화. 온보딩 답변 보관(`modu_onboarding_answers`) → 기존 계정 로그인 시 병합(같은 역할=갱신, 다른 역할=멀티프로필 추가·입장). **회원가입 탭 + 기존 계정 → "이미 모두 회원이에요" 확인 화면**(기존 계정으로 로그인 / 돌아가기) — 조용한 로그인 제거. 회귀 테스트 4건 |
| **A7 양도자 재구성** | 5e5cb3c | 진행 가이드를 CTA 바로 아래로, 지표는 접힌 묶음(매물 보유 시 자동 펼침), 매출 카드 옵트인 |
| **원격 오더 워크플로** | e04dab7 | `docs/REMOTE-WORK.md` — 모바일 원격 지시 체계. 외출 모드 배치(로컬, gitignore) |
| **CLAUDE.md 프로젝트 참고 섹션** | 1a140cc | 헌법 무변경, 아래에 추가: 자주 쓰는 명령어·폴더 구조·아키텍처 핵심(신원 모델/OAuth/멀티프로필/온보딩 데이터 흐름)·테스트 절차·작업 절차 요약 |
| **하네스 보강** | ba8b045 | SessionStart 훅(`git pull` 자동, `.claude/hooks/session-start.ps1`), Stop 완료검증 훅(미검증 코드 변경 시 "완료 보고 금지" 경고, `stop-verify.ps1` — 기존 인라인 훅 흡수), push 전 `git remote -v` 확인 규칙, 완료 검증 규칙(빌드+전체 테스트 통과 전 완료 보고 금지), `PROGRESS.md` 도입(/조각·/마감마다 갱신) |

### 이번 기간 교훈
- **"OAuth 검증은 차등 프로브로"** — 카카오는 아무 redirect_uri에도 로그인 화면을 보여줌. 등록 여부는 정상/비정상 URI 응답 차이로만 판정 가능
- **"앱 전환 왕복에는 sessionStorage가 죽는다"** — 카카오 앱으로 갔다 돌아오면 초기화. 왕복 생존 데이터는 localStorage
- **"불일치는 이벤트 대응이 아니라 구조로 막는다"** — 스와이프·뒤로가기 조합의 칩·화면 불일치는 라우트 기준 자동 동기화(useProfileRouteSync)로 종결
- **"더미 경로 테스트 통과 ≠ 실 OAuth 왕복 검증"** — 다중 선택 유실은 실기기 왕복에서만 재현. 외부 왕복이 끼는 기능은 저장소 생존까지 확인

### 다음 후보
- A3 나머지 역할(소유주·창업자·사장님·기업회원)에 업종/지역 드릴다운 확장
- E1 매물 등록에 분류 연결(listings 3컬럼 기록)
- 온보딩 답변 개인화(A7 인사말·시세 유도)
- 카드 컴포넌트 React "key prop spread" 경고 정리 (기능 영향 없음)
- 출시 전: OAuth 키 환경변수 이전, 네이버 검수 신청

---

## 이전 완료 (2026-07-08 — 통합 로그인 체계)
...
| 항목 | 파일/커밋 | 내용 |
|------|------|------|
| **A7 헤더 로고 확대** | `A7*Dashboard.jsx` (e8bf4fa, 74780c5) | 워드마크 "모두" 텍스트 제거, 심볼 크기 26→34 — 어르신 가독성 |
| **Gemini 폴백 모델 체인** | `src/lib/gemini.js` (9bd1471) | 1차 `gemini-2.5-flash-preview` 실패 시 `gemini-2.0-flash` 자동 재시도 |
| **E1 AI 초안 흐름 단순화** | `E1Step2.jsx` (2ee1d24) | 생성 완료 즉시 편집 가능 — 단계 선택(그대로/수정/공개안함) 없이 바로 수정 |
| **E1Step3 완전 제거** | `App.jsx` 외 (a48477e) | 선택 관문 화면 삭제 + 라우트·진입경로·테스트 전부 재배선 |
| **상호 공개/비공개 옵션** | `E1Step5.jsx`, `ExplorePage.jsx` (d190954) | E1 최종화면 상호 공개 토글 + 탐색 카드 마스킹 |
| **E1 흐름 4단계 재번호** | `App.jsx`, `E1Step*.jsx` 외 (fbae2b6) | `/e1/1~4` 재번호, 프로그레스바 5칸→4칸, 시설·집기 관문 제거 |
| **E1 Step2 항목별 카드 UI + item_visibility** | `E1Step2.jsx`, `E1Context.jsx`, `buildListingBlocks.js` 외 (3888829) | BlockCard: AI배지(AI작성✦), 공개/비공개 항목 토글, `item_visibility JSONB` 저장·복원·E2 비공개 체크 |
| **E1 Step2 ✏️수정 버튼 배치** | `E1Step2.jsx` (b8801e7) | 항목 헤더에 ✏️수정 버튼 명시. 클릭→textarea+저장 버튼. Playwright 144 통과 |
| **SQL 반영** | Supabase 콘솔 (대표님 직접 실행) | `ALTER TABLE listings ADD COLUMN item_visibility JSONB DEFAULT '{}'` 완료 |

### 오늘 적용된 교훈
- **"커밋 해시로 완료 허위 검증"** — 이전 세션 완료 주장한 Gemini 폴백이 실제론 미구현. `git show <hash>` 실증이 유일한 증거
- **"관문 제거 = 컴포넌트 삭제 + 라우트 제거 + 모든 진입경로 재배선"** — E1Step3 제거에서 재확인. 뒤로가기·딥링크까지 전부
- **"Playwright 통과 ≠ 실 브라우저 동작"** — ✏️수정 버튼 Playwright 통과, 실 브라우저 미표시. 원인 미파악 — 다음 세션 첫 조각

---

## 이전 완료 (2026-07-06 — DB 격리 전역 차단 + 탐색·E1 버그 + daily_contents 폴백 테스트)
...
| 항목 | 파일/커밋 | 내용 |
|------|------|------|
| 폰 테스트 환경 구축 | `모두서버시작.bat`·`모두프리뷰.bat`(로컬, gitignore — 6931fe7·5b907ec) | 더블클릭 서버 시작 배치 2종(dev 5173 / 빌드 프리뷰 4173). 한글 깨짐 → 순수 ASCII 재작성으로 해결(바이트 실측). 방화벽 5173 인바운드 허용(관리자 netsh). 폰 접속 주소 `http://192.168.219.109:5173` 실측 200 |
| **crypto.randomUUID 폴백** | `src/lib/supabase.js` (9a77ca3) | **치명 버그**: 폰(http+IP = 비보안 컨텍스트)에서 `crypto.randomUUID` 미지원 → getDeviceId throw → ErrorBoundary 전무라 A7 양도자·E1 등 **백지 크래시** (조사 에이전트 실증). getRandomValues 기반 UUID v4 폴백 추가. 회귀 2건(신규 기기 0건+device_id 필터 URL 실증 / randomUUID 제거 조건 렌더). 실 Supabase 스모크: 신규 기기 조건에서 0건·남의 매물 0·콘솔 에러 0 |
| A7 진행가이드 연극 버그 | `A7SellerDashboard.jsx` (908ee14) | E1/5 체크리스트(761f130)와 동일 패턴: GUIDE_STEPS `done:true` 하드코딩 → 매물 0건이어도 "매물 등록 ✓". buildGuideSteps(listing) 실판정으로 교체(등록=내 매물 존재·example 제외, 사진=image_urls≥3, 이후 단계는 추적 데이터 없어 미시작 고정). 미완료 사진 단계 탭 → `/e1/4?edit=<id>` 편집 진입. 테스트 4건 |
| AI 장애 완주 경로 | `E1Step2.jsx`, `E1Step5.jsx` (56713d9) | Gemini 502 1시간 장애 계기(curl 판정: 키·모델·엔드포인트 유효, 구글 측 일시 장애 — 현재 회복). E1/5 가드 `!aiDraft`(AI 죽으면 완주 불가) → `상호명·주소 없음`(빈 매물 방지 목적 유지)으로 교체. E1/2 실패 안내 정직화("AI 초안 생성이 지금 안 돼요" + "AI 없이 계속 진행" 버튼). ai_draft null 저장은 E2 `\|\| {}` 안전 확인. 테스트 3건 |
| A7 "남의 매물" 신고 전수 스캔 | (조사만 — 코드 무변경) | `from('listings')` 8곳 전수: A7 내 매물에 남의 매물 나올 코드 경로 **없음**(필터·폴백·캐시·realtime 전부 확인). 아이폰 관찰은 탐색/창업피드(published 전체 = 정상 설계) 혼동 또는 기존 device_id 보유 기기로 추정 — 재현 시 "화면/단계+현상"으로 판정 |
| 권한 정비 | `.claude/settings.local.json`(로컬) | allow 27개로 정비 — 조사용 읽기 전용(curl·cat·ls·head·tail·wc·netstat·ipconfig + PowerShell 네트워크 진단 5종) 추가, 일회성 승인 잔재 8개 흡수. deny(git push·rm) 유지. 한계 고지: 접두사 매칭이라 curl GET/DELETE 구분 불가 — 변조 호출 금지는 헌법으로 지킴 |
| **미완 (솔직 고지)** | — | **Daum 팝업 직접 타이핑·사진 업로드→E2 표시·E1 완주(폰 실기기) 검증 미실시** — 코워크 우회 구간 그대로 남음. 다음 세션(7/6 시뮬레이션) 최우선 |

## 이전 완료 (2026-07-05 — 정직화 마감 + 시뮬 준비)
...
| `/e1/1~4` | 양도자 매물 등록 4단계 | ✅ 완성 | AI 초안 Gemini 실연결, item_visibility 저장 |
...
| 우선순위 | 항목 | 내용 |
|----------|------|------|
| 🔴 **1순위** | **A1~A4 진입 흐름 전면 재검토** | 대표님 지정. 스플래시→카테고리→온보딩→로그인 전체 UX를 처음부터 다시 점검 |
| 🔴 | **로그인 체계 잔여 (서비스 오픈 준비 단계로 보류)** | ①Supabase Auth 콘솔 설정(Confirm email OFF·Redirect URL) ②실연동 검수(카카오·이메일 로그인 스크린샷 확인) ③네이버 구현 — 방안 보고 완료, 대표님 네이버 개발자센터 앱 등록 후 착수 ④conversations.sender/receiver_user_id REFERENCES 콘솔 적용 여부 확인 ⑤device_id→user 마이그레이션 실행 시점 결정 ⑥Magic Link 단일화 vs 이메일+비밀번호 유지 결정(대표님 선택) |
| 🔴 | **개발용 계정 스위처** | 오더 문안 확정. 카테고리별 6인격 빠른 전환 UI — D4 설계 세션 선행 |
| 🔴 | **시설·집기 빨간펜** | E1Step3(구 E1Step4) 시설·집기 항목 리뷰 |
| 🔵 | **Playwright viewport 390px 전역 고정** | playwright.config.js 전역 고정 — 일부 테스트만 390px 적용 중 |
| 🔵 | D4 설계 세션 | 스위처 완료 후 별도 설계 세션 |
| 🔵 | D4 실 상대방 수락 흐름 | 연락처 교환 "수락"이 요청자 화면의 더미 버튼 — 상대방 화면 수락/거절 UI + Realtime |
| ⚪ | 정식 RLS | device_id→auth 전환과 함께 (현재 울타리 수준) |
| ⚪ | API 키 서버사이드 프록시 이전 | 출시 전 Edge Function 이전 + 키 재발급 |
| ⚪ | 커뮤니티 오픈채팅 실연결 | 그룹 채팅 데이터 모델 설계부터 |
| ⚪ | A7 준비중 섹션 실데이터 연동 | 시장동향·거래처·콘텐츠·매출 |
| ⚪ | D4BusinessChat 매칭성사 실연결 | 보존된 B2B UI를 실연결 채팅에 얹기 |
| ⚪ | 빈 점포 DM + 카드 더미 (E2L 포함) | 임대인 E1p 실연결 선행 필요 |
| ⚪ | Storage 고아 파일 정책 | 매물 완전삭제 기능 도입 때 함께 결정 |
| ⚪ | 수정 모드 전용 draft | 새로고침 시 수정 진행 내용 소실 |

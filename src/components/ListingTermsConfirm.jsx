...
export default function ListingTermsConfirm({ terms, agreed, onToggle, accent = '#1a4d8f' }) {
  return (
    <div className="mb-4" data-testid="listing-terms">
      <p className="text-[13px] font-bold text-gray-900 mb-2">등록 확인사항</p>
      <div className="rounded-2xl border border-gray-200 bg-gray-50/60 px-4 py-3 max-h-44 overflow-y-auto">
        <ol className="space-y-2">
          {terms.map((t, i) => (
            <li key={i} className="flex gap-1.5 text-[11px] leading-relaxed text-gray-500">
      ...
      <label className="flex items-start gap-2.5 mt-3 cursor-pointer select-none" data-testid="terms-agree-row">
        <input
          ...
          style={{ color: accent }}
        />
        <span className="text-[12px] font-semibold text-gray-700 leading-snug">{TERMS_CHECKBOX_LABEL}</span>
      </label>
      {!agreed && (
        <p className="text-[11px] text-gray-400 mt-1.5 ml-6" data-testid="terms-agree-hint">

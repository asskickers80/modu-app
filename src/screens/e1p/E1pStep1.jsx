    ...
    <div className="h-screen flex flex-col overflow-hidden">
      ...
      <div className="shrink-0 bg-white">
        <div className="flex items-center px-5 pt-12 pb-2 gap-2">
          ...
          <span className="text-t13 font-bold shrink-0" style={{ color: TEAL }}>1 / 4</span>
        </div>
        <ProgressBar />
        ...
        <div className="px-5 pb-5 border-b border-gray-50">
          <h2 className="text-t20 font-bold text-gray-900">상가 정보를 입력해요</h2>
          <p className="text-t13 text-gray-400 mt-1">주소만 넣으면 대부분 자동으로 채워져요 (예정)</p>
        </div>
      </div>
      ...
      <main className="flex-1 overflow-y-auto px-5 pb-32" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className="mt-5 mb-6">
          <p className="text-t14 font-bold text-gray-900 mb-3">어떤 목적으로 내놓을까요?</p>
            ...
            {LISTING_TYPES.map(opt => {
              ...
              return (
                <button key={opt.id}
                  ...
                  <span className="text-t20">{opt.emoji}</span>
                  <span className="text-t14 font-bold" style={{ color: sel ? TEAL : '#111827' }}>
                    {opt.label}
                  </span>
                  <span className="text-t10" style={{ color: sel ? TEAL : '#9ca3af' }}>
                    {opt.sub}
                  </span>
                </button>
              )
        ...
        <div className="mb-5">
          <p className="text-t14 font-bold text-gray-900 mb-3">주소</p>
        ...
        {data.address && (
          <div className="mb-5">
            <p className="text-t14 font-bold text-gray-900 mb-3">층수 · 면적</p>
              ...
              {FLOOR_OPTS.map(f => {
                ...
                return (
                  <button key={f}
                    ...
                    className="px-4 py-2 rounded-full text-t13 font-medium border transition-all active:scale-95"
                    style={{
                      borderColor: sel ? TEAL : '#e5e7eb',
                      backgroundColor: sel ? TEAL_BG : '#f9fafb',
            ...
            <p className="text-t12 text-gray-400 mb-1.5">전용 면적</p>
            <div className="flex items-center gap-2 border rounded-xl px-4 py-3"
        ...
        {data.address && data.listingType && (
          <div className="mb-5">
            <p className="text-t14 font-bold text-gray-900 mb-1">
              ...
              {[{ id: 'occupied', label: '있어요', sub: '현 계약 기준' }, { id: 'vacant', label: '없어요', sub: '예상(시세) 기준' }].map(o => {
                ...
                return (
                  <button key={o.id} data-testid={`occ-${o.id}`}
                    ...
                    <span className="text-t14 font-bold" style={{ color: sel ? TEAL : '#111827' }}>{o.label}</span>
                    <span className="text-t10" style={{ color: sel ? TEAL : '#9ca3af' }}>{o.sub}</span>
                  </button>
                )
              })}
            </div>
        ...
        {data.address && isRent && (
          <div className="mb-5">
            <p className="text-t14 font-bold text-gray-900 mb-3">임대 조건</p>
        ...
        {data.address && isSale && (
          <div className="mb-5">
            <p className="text-t14 font-bold text-gray-900 mb-3">매각 조건</p>
            <div className="flex flex-col gap-3">
              ...
              <div className="rounded-xl px-4 py-3" style={{ backgroundColor: TEAL_BG, border: `1px solid ${TEAL}25` }}>
                <div className="flex items-center justify-between">
                  <p className="text-t13 font-semibold" style={{ color: TEAL }} data-testid="yield-label">{yieldLabel}</p>
                  <p className="text-t16 font-black" data-testid="yield-value" style={{ color: TEAL }}>
                    {computedCap != null ? `${computedCap}%` : '—'}
                  </p>
                </div>
                <p className="text-t11 text-gray-400 mt-1">
        ...
        {data.address && isRent && (
          <div className="mb-5">
            <p className="text-t14 font-bold text-gray-900 mb-1">선호 업종 (선택)</p>
            <p className="text-t12 text-gray-400 mb-3">어떤 업종 임차인을 선호하시나요?</p>

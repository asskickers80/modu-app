...
const CATS = ['전체', '카페', '식당', '미용', '기타']
const TYPES = ['전체', '영업양도', '바닥권리', '프랜차이즈']

// ── 하단 네비 아이콘 ───────────────────────────────────────
const icons = {
  home: c => <svg width="22" height="22" viewBox="0 0 22 22" fill="none"><path d="M3 9.5L11 3l8 6.5V19a1 1 0 01-1 1H4a1 1 0 01-1-1V9.5z" stroke={c} strokeWidth="1.6" strokeLinejoin="round" fill={c === NAVY ? NAVY_BG : 'none'} /><path d="M8 20v-7h6v7" stroke={c} strokeWidth="1.6" strokeLinejoin="round" /></svg>,
...
function NavBar({ navigate, showToast }) {
  const tabs = [
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-start justify-between">
          ...
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full shrink-0 ml-2 mt-0.5"
            style={{ backgroundColor: NAVY_BG, color: NAVY }}>{item.type}</span>
        </div>
        <div className="flex items-center gap-3 mt-2">
          <span className="text-[12px] font-bold" style={{ color: NAVY }}>권리금 {item.fee.toLocaleString()}만</span>
      ...
      <main className="flex-1 overflow-y-auto" style={{ scrollbarWidth: 'none' }}>
        <div className="px-4">
          <div className="flex items-center justify-between py-3">
            ...
            <button onClick={() => showToast()} className="text-[12px] font-medium" style={{ color: NAVY }}>
              필터·정렬
            </button>
          </div>
          {filtered.map(item => (
            <PropertyCard key={item.id} item={item} onClick={() => navigate(`/e2/${item.id}`)} />
          ))}
          <div className="h-6" />
        </div>
      </main>

      <NavBar navigate={navigate} showToast={showToast} />
      <Toast message={toast} />
    </div>
  )
}

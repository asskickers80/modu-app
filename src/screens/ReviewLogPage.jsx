    ...
    <div className="rounded-2xl border border-gray-200 overflow-hidden">
      ...
      <button
        ...
        <div className="flex-1 min-w-0">
          <p className="text-t14 font-bold text-gray-900 truncate">
            {listing.shopName || '(상호 미입력)'}
          </p>
          <p className="text-t12 text-gray-400 mt-0.5 truncate">
            {listing.address || '주소 없음'} · {TRANSFER_LABEL[listing.transferType] ?? '-'}
          </p>
          <div className="flex items-center gap-2 mt-2">
            <span className="text-t11 text-gray-400">{dateStr}</span>
            <span className="text-gray-200">|</span>
            <span className="text-t11 font-semibold text-green-600">그대로 {summary.kept}</span>
            <span className="text-t11 font-semibold text-blue-500">수정 {summary.edited}</span>
            <span className="text-t11 font-semibold text-gray-400">비공개 {summary.hidden}</span>
          </div>
        </div>
      ...
      {open && (
        <div className="border-t border-gray-100 bg-gray-50 px-4 py-3 flex flex-col gap-3">
          {items.map(item => (
            <div key={item.blockId} className="bg-white rounded-xl p-3 flex flex-col gap-2">
              <div className="flex items-center gap-2">
                <p className="text-t12 font-bold text-gray-700 flex-1">{item.blockTitle}</p>
                <ChoiceBadge choice={item.userChoice} />
              </div>

              {/* AI 원본 */}
              <div>
                <p className="text-t10 font-bold text-gray-400 mb-1">모두 원본</p>
                <p className="text-t12 text-gray-600 leading-relaxed whitespace-pre-line line-clamp-3">
                  {item.aiOriginal}
                </p>
              </div>
              ...
              {item.userChoice === 'edit' && item.userText && (
                <div className="border-t border-dashed border-blue-200 pt-2">
                  <p className="text-t10 font-bold text-blue-400 mb-1">사람이 수정한 텍스트</p>
                  <p className="text-t12 text-blue-700 leading-relaxed whitespace-pre-line">
                    {item.userText}
                  </p>
                </div>
              )}
              ...
              {item.userChoice === 'hide' && (
                <p className="text-t11 text-gray-400 border-t border-gray-100 pt-2">
          ...
          <details className="bg-white rounded-xl p-3">
            <summary className="text-t11 font-bold text-gray-400 cursor-pointer select-none">
              매물 입력 데이터 (원본) 보기
            </summary>
            <pre className="mt-2 text-t10 text-gray-500 leading-relaxed overflow-auto">
    ...
    <div className="min-h-screen pb-10" style={{ backgroundColor: '#0f172a' }}>
      ...
      <div className="px-5 pt-12 pb-5">
        <button onClick={() => navigate('/dev')}
          ...
          <span className="text-t13">Dev 메뉴로</span>
        </button>

        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full mb-3"
          style={{ backgroundColor: '#1e293b' }}>
          <span className="w-2 h-2 rounded-full bg-green-400" />
          <span className="text-t11 font-bold text-green-400 tracking-wide">모두 학습 재료</span>
        </div>
        <h1 className="text-t22 font-black text-white mb-1">검수 로그</h1>
        ...
        <div className="px-5 mb-4 flex gap-2 flex-wrap">
          {[
            ...
            <div key={stat.label} className="px-3 py-2 rounded-xl" style={{ backgroundColor: '#1e293b' }}>
              <p className="text-t10 text-slate-400">{stat.label}</p>
              <p className="text-t18 font-black" style={{ color: stat.color }}>{stat.value}</p>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="px-4 flex flex-col gap-3">
        {logs.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
            ...
            <p className="text-t16 font-bold text-slate-300">아직 검수 기록이 없어요</p>
            <p className="text-t13 text-slate-500 leading-relaxed">
              E1 1단계 → 2단계(초안 생성) → 3단계(검수) 를<br />완료하면 여기에 기록이 쌓여요
            </p>
            <button onClick={() => navigate('/e1/1')}
              className="mt-2 px-5 py-3 rounded-2xl text-t14 font-bold text-white"
        ...
        <div className="px-5 mt-6">
          {!confirmClear ? (
            <button onClick={() => setConfirmClear(true)}
              className="w-full py-3 rounded-2xl text-t13 font-semibold text-red-400 border border-red-900">
              전체 로그 삭제 (테스트용)
            </button>
          ) : (
            <div className="flex gap-2">
              <button onClick={() => setConfirmClear(false)}
                className="flex-1 py-3 rounded-2xl text-t13 font-semibold text-slate-400 border border-slate-700">
                취소
              </button>
              <button onClick={handleClear}
                className="flex-1 py-3 rounded-2xl text-t13 font-bold text-white bg-red-600">
                확인 — 전체 삭제
              </button>
            </div>
          )}

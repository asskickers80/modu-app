...
export default function AuthResetPasswordPage() {
  ...
  if (done) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 bg-white px-8">
        ...
        <p className="text-t18 font-bold text-gray-900 text-center">비밀번호가 변경됐어요</p>
        <p className="text-t13 text-gray-400 text-center">새 비밀번호로 로그인해 주세요</p>
      </div>
    )
  }
  ...
  if (!ready) {
    return (
      <div className="h-screen flex flex-col items-center justify-center gap-4 bg-white">
        ...
        <p className="text-t14 text-gray-500">링크를 확인하는 중...</p>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col min-h-screen px-5 pt-20 pb-8">
      ...
      <p className="text-t14 text-gray-400 mb-8">6자 이상으로 설정해 주세요</p>
      <div className="flex flex-col gap-3">
        <input
          ...
          className="w-full px-4 py-[14px] rounded-2xl border border-gray-200 text-t14 outline-none focus:border-gray-400"
        />
        <input
          type="password"
          value={confirm}
        ...
        <button
          ...
          className="w-full py-[16px] rounded-2xl text-t15 font-bold text-white disabled:opacity-50 transition-all"

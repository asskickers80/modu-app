        ...
        <div className="flex items-center gap-3 px-4 pt-12 pb-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <p className="text-t15 font-bold text-gray-900 truncate">{thread.clientName}</p>
            ...
            <p className="text-t11 text-gray-400 mt-0.5 truncate">🔨 {thread.propertyName}</p>
          </div>
        </div>
        <div className="mx-4 mb-3 flex items-center gap-2 px-3 py-2 rounded-xl" style={{ backgroundColor: PURPLE_BG }}>
          <p className="text-t11 font-medium flex-1" style={{ color: PURPLE }}>대화 중 · 번호는 비공개예요 — 연락처 교환 시 양측 동시 공개</p>
        </div>
      </header>
            ...
            <div key={msg.id}>
              {showDate && (
                <div className="flex items-center gap-3 my-3">
                  ...
                  <span className="text-t11 text-gray-400">{msg.date}</span>
                  <div className="flex-1 h-px bg-gray-100" />
                </div>
              )}
              <div className={`flex mb-2 ${isBiz ? 'justify-end' : 'justify-start'}`}>
                {!isBiz && (
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-t11 font-bold text-white mr-2 mt-1 shrink-0"
                ...
                <div className={`flex flex-col ${isBiz ? 'items-end' : 'items-start'}`}>
                  <div className="max-w-[240px] px-3.5 py-2.5"
                    ...
                    <p className="text-t14 leading-relaxed" style={{ color: isBiz ? 'white' : '#111827' }}>{msg.text}</p>
                  </div>
                  <span className="text-t10 text-gray-400 mt-1 mx-1">{msg.time}</span>
                </div>
              </div>
            </div>
          )
        ...
        {exchangeState === 'pending' && (
          <div className="mx-4 mb-3 rounded-2xl border-2 p-4" style={{ borderColor: PURPLE + '40', backgroundColor: PURPLE_BG }}>
            <div className="flex items-center gap-2 mb-2">
              ...
              <p className="text-t13 font-bold" style={{ color: PURPLE }}>연락처 교환 요청 보냄</p>
            </div>
            <p className="text-t12 text-gray-500 mb-1">상대방이 수락하면 양쪽 번호가 동시에 공개됩니다.</p>
            {/* 기업 축은 실데이터 미연결 — 수락 응답은 실연결 후 동작 ((예정), 더미 수락 버튼 제거) */}
            <p className="text-t11 text-gray-400">상대 수락 응답 연결 준비 중이에요 (예정)</p>
          </div>
        )}
        {exchangeState === 'accepted' && (
          <div className="mx-4 my-3 rounded-2xl border-2 overflow-hidden" style={{ borderColor: PURPLE + '40' }}>
            ...
            <div className="px-4 py-3 flex items-center gap-2" style={{ backgroundColor: PURPLE_BG }}>
              ...
              <div>
                <p className="text-t13 font-black" style={{ color: PURPLE }}>매칭 성사</p>
                <p className="text-t11 text-gray-500">연락처가 교환됐어요 · 이제 직접 연락 가능</p>
              </div>
            </div>
            <div className="px-4 py-3.5 bg-white flex gap-4">
              <div className="flex-1 rounded-xl px-3 py-2.5" style={{ backgroundColor: PURPLE_BG }}>
                <p className="text-t10 text-gray-400 mb-1">{thread.clientName}</p>
                <p className="text-t15 font-bold tracking-wide" style={{ color: PURPLE }}>{thread.clientPhone}</p>
              </div>
              <div className="flex-1 rounded-xl px-3 py-2.5 bg-gray-50">
                <p className="text-t10 text-gray-400 mb-1">내 번호 (상대 공개)</p>
                <p className="text-t15 font-bold tracking-wide text-gray-700">{thread.bizPhone}</p>
              </div>
            </div>
          </div>
        )}
        ...
        {exchangeState === 'idle' && (
          <div className="px-4 pt-3">
            <button onClick={() => setShowModal(true)}
              className="w-full py-2.5 rounded-xl text-t13 font-bold border-2 flex items-center justify-center gap-2"
              style={{ borderColor: PURPLE + '50', color: PURPLE, backgroundColor: PURPLE_BG }}>
              📇 연락처 교환 요청 <span className="text-t10 opacity-70">(양측 합의 필요)</span>
            </button>
          </div>
        )}
        {exchangeState === 'accepted' && (
          <div className="px-4 pt-3">
            <div className="w-full py-2.5 rounded-xl text-t13 font-bold flex items-center justify-center"
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center">
          ...
          <div className="relative w-full max-w-[390px] bg-white rounded-t-3xl px-5 pt-5 pb-10 shadow-2xl">
            ...
            <p className="text-t17 font-bold text-gray-900 mb-1">연락처 교환 요청</p>
            <p className="text-t12 text-gray-400 mb-4">{thread.clientName}에게 요청을 보냅니다</p>
            <div className="rounded-2xl p-4 mb-5" style={{ backgroundColor: '#fff8e1' }}>
              ...
              <ul className="space-y-1.5">
                {['교환하면 양쪽 모두 전화번호가 공개됩니다', '상대방이 수락해야만 공개돼요 (일방 공개 불가)'].map(t => (
                  <li key={t} className="flex items-start gap-2 text-t12 text-amber-800">

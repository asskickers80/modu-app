    ...
    <div className="h-screen flex flex-col">
      <header className="shrink-0 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3 px-4 pt-12 pb-4">
          ...
          <h1 className="text-t18 font-bold text-gray-900">인증 진행 중</h1>
        </div>
      </header>
      <main className="flex-1 flex flex-col items-center justify-center px-6">
        <p className="text-[40px] mb-4">{METHODS.find(m => m.id === selectedMethod)?.emoji}</p>
        <p className="text-t18 font-bold text-gray-900 mb-2 text-center">
          {METHODS.find(m => m.id === selectedMethod)?.label}
        </p>
        <p className="text-t13 text-gray-500 text-center mb-8 leading-relaxed">
        ...
        {selectedMethod === 'phone' && (
          <div className="w-full mb-6">
            <input type="number" inputMode="numeric" placeholder="인증번호 6자리"
              className="w-full text-center text-t20 font-bold py-4 rounded-2xl border border-gray-200 outline-none bg-white" />
          </div>
        )}
    ...
    <div className="h-screen flex flex-col">
      ...
      <header className="shrink-0 bg-white border-b border-gray-100">
        <div className="flex items-center gap-3 px-4 pt-12 pb-4">
          ...
          <h1 className="text-t18 font-bold text-gray-900">인증 필요</h1>
        </div>
      </header>
      ...
      <main className="flex-1 overflow-y-auto px-4 py-6" style={{ scrollbarWidth: 'none' }}>
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-t20 font-bold text-gray-900 mb-2">{gate.title}</p>
          <p className="text-t14 text-gray-500 leading-relaxed">{gate.desc}</p>
          <div className="mt-4 mx-4 rounded-2xl px-4 py-3" style={{ backgroundColor: '#eef2fb' }}>
            <p className="text-t12 text-gray-600">
              💡 {gate.reason}
            </p>
          </div>
        </div>
        ...
        {/* 인증 방법 선택 */}
        <p className="text-t13 font-bold text-gray-700 mb-3">인증 방법 선택</p>
          ...
          {METHODS.map(m => (
            <button key={m.id}
              ...
              <div className="flex-1">
                <p className="text-t14 font-bold text-gray-900">{m.label}</p>
                <p className="text-t12 text-gray-400">{m.desc}</p>
              </div>
        ...
        <button onClick={() => navigate(-1)}
          className="w-full py-3 text-t13 text-gray-400 mt-3">
          나중에 하기
        </button>
      </main>
